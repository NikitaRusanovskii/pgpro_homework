...
Обновлено 3: Иван Рогов в книге "PostgreSQL изнутри" описывает shared_buffers на страницах 194-196. Там сказано, что оптимальный размер буффера зависит от каждой системы индивидуально. Судя по тому, что видно в приведённых результатах, буффер размером 1024 MB оказывался слишком большим, поэтому там могло плодиться большое количество старых версий строк, что сказалось на производительности. Ведь на обработку буфферов тоже необходимо выделять ресурсы. Однако из-за "нестерильных" условий теста, который я проводил, утверждать однозначно ничего нельзя. В книге предлагается использовать pg_stat_io и pg_buffercache или же pg_buffercache_summary и pg_buffercache_usage_counts для анализа работы буфферов.

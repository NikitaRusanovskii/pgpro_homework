...
В ходе работы над задачей ознакомился с утилитой pgbench для нагрузочного тестирования. Узнал, как можно конфигурировать кластер из под psql (ALTER SYSTEM), написал удобный скрипт, позволяющий в кратчайшие сроки поднять окружение для нагрузочного тестирования. Узнал о лимите на подключения к кластеру, а также рассмотрел, как параметр shared_buffers влияет на производительность. Сравнил версию, которую я использую с версией 20devel, наблюдалась заметная разница в производительности.

Обновлено: в программе автоматизации тестирования был найден критический баг, из-за которого вышла путаница с наименованием графиков. Графики подписаны наоборот (те, что с pg_stat_statements=yes, на самом деле, без библиотеки). Работаю над исправлением.

Обновлено 2: Баг исправил. Также исправил баг с тем, что после сборки 17.11 в репозитории с исходниками оставались мусорные объекты, влияние которых я не учёл. Обновил тесты, их графики и отчёт. На Github выложена актуальная версия отчёта.

Обновлено 3: Иван Рогов в книге "PostgreSQL изнутри" описывает shared_buffers на страницах 194-196. Там сказано, что оптимальный размер буффера зависит от каждой системы индивидуально. Судя по тому, что видно в приведённых результатах, буффер размером 1024 MB оказывался слишком большим, поэтому там плодилось большое количество старых версий строк. На обработку буфферов тоже необходимо выделять ресурсы. Возможно, поэтому были заметны просадки по производительности. Однако из-за "нестерильных" условий теста, который я проводил, утверждать однозначно ничего нельзя. В книге предлагается использовать pg_stat_io и pg_buffer_cache или же pg_buffercache_summary и pg_buffercache_usage_counts для анализа работы буфферов.
...
После исправления критического бага картина стала ожидаемой: на большом количестве подключений включение библиотеки pg_stat_statements оказывало неблагоприятное влияние на производителность. TPS падал примерно на 10%, как видно из графиков. Однако такое поведение может быть также связано с троттлингом процессора при перегреве. Поэтому тестирование лучше проводить на независимой и хорошо охлаждаемой машине. Небольшими партиями, в случайном порядке конфигурации. Тогда шум будет распреляться равномерно на все входные данные, и это позволит организовать тестирование более честным образом.

Исходя из графиков, при количестве подключений > 16 по производительности выбивается конфигурация с выключенной библиотекой и shared_buffers=512MB (PostgreSQL, 17.11), остальные конфигурации имеют схожее поведение. PostgreSQL, 20devel: конфигурация без библиотеки и с shared_buffers=1024 при количестве соединений > 16 теперь выглядит более производительной, занимает второе место по количеству TPS, однако конфигурация shared_buffers=512MB без библиотеки показала себя лучше.  

    После сравнения результатов тестирования, можно сказать, что в последняя версия (20devel) показывает себя лучше при некоторых конфигурациях тестирования. Вероятно, между 17 и 20devel версиями была проделана работа по увеличению производительности
